...
def load_1_DE_model(ckpt_path, full_path=True, mem_id=0):
    model1 = models.resnet50().cuda()
    # if not full_path:
        # raise
        # sd = torch.load(f"./dist_models/{ckpt_path}/checkpoint_best.pth", map_location="cpu")
    # else:
    sd = torch.load(ckpt_path, map_location="cpu")
    ckpt = {k.replace("encoders.",""):v for k,v in sd['enc_state_dict'].items()}
    fil_ckpt = {k:v for k,v in ckpt.items() if f'member{mem_id}' in k}
    cls_ckpt =  {k.replace("branches.","fc."):v for k,v in sd['class_state_dict'].items()}
    cls_fil_ckpt = {k:v for k,v in cls_ckpt.items() if f'member{mem_id}' in k}
    fil_ckpt.update(cls_fil_ckpt)
    fil_ckpt = {k.replace(f"member{mem_id}.",""):v for k,v in fil_ckpt.items()}
    model1.load_state_dict(fil_ckpt)
    print(f"loaded {ckpt_path}")
    model1.eval()
    return model1
...
M1_filepath = "/gpfs/u/home/BNSS/BNSSlhch/scratch/ensem_ssl/dist_models/sup_DE_ckpts/imagenet_M1_proj_sharedFalse_sepFalse_NCEFalse_t0.1_l0.1__bs4080_lr1.6_model_best.pth.tar"
M1 = load_1_DE_model(M1_filepath, full_path=True, mem_id=0)
m1_out, m1_tar = rollout_loader(M1, test_loader2)
print(m1_out.shape, m1_tar.shape)
torch.save(m1_tar.cpu(), "./logits_files/M_targets.pth")
torch.save(m1_out.cpu(), "./logits_files/m1.pth")

M2_filepath = "/gpfs/u/home/BNSS/BNSSlhch/scratch/ensem_ssl/dist_models/sup_DE_ckpts/imagenet_M2_proj_sharedFalse_sepFalse_NCEFalse_t0.1_l0.1_bs4080_lr1.6_model_best.pth.tar"
M2 = load_1_DE_model(M2_filepath, full_path=True, mem_id=0)
m2_out, m2_tar = rollout_loader(M2, test_loader2)
print(m2_out.shape, m2_tar.shape)
torch.save(m2_out.cpu(), "./logits_files/m2.pth")


M3 = load_1_DE_model(M2_filepath, full_path=True, mem_id=1)
m3_out, m3_tar = rollout_loader(M3, test_loader2)
print(m3_out.shape, m3_tar.shape)
torch.save(m3_out.cpu(), "./logits_files/m3.pth")


all_M = [m1_out,m2_out,m3_out]
tars = [m1_tar, m2_tar, m3_tar]
names = ['m1', 'm2', 'm3']
get_metrics(all_M,tars,names)
#
