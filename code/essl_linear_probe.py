...
parser.add_argument('--save-freq', default=10, type=int, metavar='N',
                    help='save frequency')
parser.add_argument('--exp-id', type=str, help='id/name for exp')



def main():
    args = parser.parse_args()
    Path(f'./checkpoint/{args.exp_id}').mkdir(parents=True, exist_ok=True)
    args.checkpoint_dir = f'{os.getcwd()}/checkpoint/{args.exp_id}'
...
def main_worker(gpu, args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.rank == 0:
            ...
            torch.save(state, f'{args.checkpoint_dir}/checkpoint.pth')

            # save checkpoint to epoch
            if epoch % args.save_freq == 0 and epoch != 0:
                torch.save(state, f'{args.checkpoint_dir}/checkpoint_epoch{epoch}.pth'  )
