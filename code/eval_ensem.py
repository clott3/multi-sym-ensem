...
# except ImportError:
#     from torchmetrics_classification import MulticlassPrecision, MulticlassRecall, MulticlassAUROC
...
parser.add_argument('--dataset', type=str, default='imagenet',
                    ...
                    'aircraft', 'voc-2007', 'dtd', 'caltech-101', 'ftcifar10', 'ftcifar100'),
                    help='dataset name')
...
parser.add_argument('--arch', default='resnet50', type=str,
                    choices=('resnet18', 'resnet50', 'cifar_resnet18'),
...
# using different splits of in-1000 (in-100, in-200, in-500, etc)
...
def main():
    ...
    if args.submit:
        ...
        return
    print(args.use_default_pretrained, args.dataset, args.server)
    ...
    if args.eval_subset100:
        ...
        assert 'ImageNet' in str(args.data)
...
def main_worker(gpu, ngpus_per_node, args):
    if 'ftcifar' in args.dataset:
        args.data = '/home/gridsan/cloh/MAML-Soljacic/cifar_stl_data'
        args.arch = 'cifar_resnet18'
    ...
    if args.use_smaller_split:
        args.ds_num_classes = args.val_perc
    else:
    ...
    if (args.checkpoint_dir / 'checkpoint.pth').is_file():
        ...
        num_ensem = len(args.pretrained)
        args.num_ensem = num_ensem
        model = EnsembleSSL(args.arch, args.num_ensem, args.ds_num_classes, args.eval_mode).cuda()
        start_epoch = 0
    ...
    # create dataloaders
    train_loader, val_loader, test_loader, indices_in_1k = build_dataloaders(args)
    if args.eval_mode != 'freeze':
        print("Checking train transform:")
        if train_loader.dataset.__class__ == Subset:
            print(train_loader.dataset.dataset.transform)
        else:
        # elif train_loader.dataset.__class__ == ImageFolder or train_loader.dataset.__class__ == CIFAR10 or train_loader.dataset.__class__ == CIFAR100:
            print(train_loader.dataset.transform)
        # else:
            # "cannot check transform"

    if val_loader is not None:
        print("Checking val transform:")
        if val_loader.dataset.__class__ == Subset:
            print(val_loader.dataset.dataset.transform)
        else:
        # elif val_loader.dataset.__class__ == ImageFolder or val_loader.dataset.__class__ == CIFAR10 or val_loader.dataset.__class__ == CIFAR100:
            print(val_loader.dataset.transform)
        # else:
            # "cannot check transform"

    if test_loader is not None:
        print("Checking test transform:")
        if test_loader.dataset.__class__ == Subset:
            print(test_loader.dataset.dataset.transform)
        else:
            print(test_loader.dataset.transform)

        # elif test_loader.dataset.__class__ == ImageFolder or test_loader.dataset.__class__ == CIFAR10 or test_loader.dataset.__class__ == CIFAR100:
            # print(test_loader.dataset.transform)
        # else:
            # "cannot check transform"
    ...
    if args.eval_mode in {'freeze', 'log_reg'}:
        start_epoch = 0
        args.epochs = 1
...
def validate(model, val_loader, logger, indices_in_1k, epoch, args, best_acc):
    ...
    # auroc_meter = AverageMeter('AUROC@1')

    # per class accuracy
    # if val_loader.dataset.__class__ == ImageFolder:
    ...
    # define precision, recall, auroc metrics
    print("num classes: ", num_classes)
    # precision = Precision(task="multiclass", average='macro', num_classes=num_classes)
    # precision = MulticlassPrecision(num_classes=num_classes, average='none').cuda()
    # recall = MulticlassRecall(num_classes=num_classes, average='none').cuda()
    # auroc = MulticlassAUROC(num_classes=num_classes, average='none').cuda()
    ...
    # prec_per_class = torch.zeros(num_classes, device='cuda')
    # rec_per_class = torch.zeros(num_classes, device='cuda')
    # auroc_per_class = torch.zeros(num_classes, device='cuda')
    ...
    with torch.no_grad():
        for images, target in tqdm(val_loader):
            ...
            if len(args.weighting) > 0:
                weighting = torch.Tensor(args.weighting).repeat(len(target),1).cuda()
                logit = weighting.T.unsqueeze(2).repeat(1,1,args.ds_num_classes) * output.softmax(dim=-1)
                output_mean = logit.sum(dim=0)
            else:
                output_mean = output.softmax(dim=-1).mean(dim=0)
            # print(time.time() - time0)
            preds, new_target, _, acc_ub, mask = ensem_pred(output, mode=args.ensem_pred, target=target)
            sys.stdout.flush()
            ...
            # print(prec)
            # precision_meter.update(torch.mean(prec).item(), images.size(0))
            # recall_meter.update(torch.mean(rec).item(), images.size(0))
            # auroc_meter.update(torch.mean(auro).item(), images.size(0))
...
def make_sh_and_submit(args, delay=0):
    ...
    with open(f'./scripts/submit_scripts/{name}_{delay}.sh', 'w') as file:
        ...
        if args.server == 'sc' or args.server == 'rumensc':
            if args.dataset == 'inat-1k':
                file.write(f'--data=/home/gridsan/groups/MAML-Soljacic/inat-1k/ --dataset=inat-1k ')
            elif args.dataset == 'food':
                file.write(f'--data=/home/gridsan/groups/MAML-Soljacic/food-101/ --dataset=food ')
