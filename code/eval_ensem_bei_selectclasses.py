...
parser.add_argument('--use_bei_labels', action='store_true')
parser.add_argument('--hard_bei', action='store_true')
parser.add_argument('--uniq_bei', action='store_true')
parser.add_argument('--non_uniq_bei', action='store_true')
parser.add_argument('--select_classes', default='', type=str)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create dataloaders
    train_loader, val_loader, indices_in_1k = build_dataloaders(args)
...
def validate(model, val_loader, logger, indices_in_1k, epoch, args, best_acc):
    ...
    if args.use_bei_labels:
        if args.hard_bei:
            with open("./misc/imagenet_class_to_bei_hardlabel_val.json") as jfile:
                file_contents = jfile.read()
                bei_dict = json.loads(file_contents)

        else:
            with open("./misc/imagenet_class_to_bei_label_val.json") as jfile:
                file_contents = jfile.read()
                bei_dict = json.loads(file_contents)

    elif args.uniq_bei:
        with open("./misc/imagenet_class_to_bei_ind_uniq_val.json") as jfile:
            file_contents = jfile.read()
            bei_dict = json.loads(file_contents)
    elif args.non_uniq_bei:
        with open("./misc/imagenet_class_to_bei_label_val.json") as jfile:
            file_contents = jfile.read()
            bei_dict = json.loads(file_contents)
    else:
    ...
    if args.select_classes:
        selected_classes = np.load(f"./misc/{args.select_classes}.npy")
    else:
    ...
    uniq_correct = torch.zeros(3, device='cuda')
    uniq_total = torch.zeros(3, device='cuda')
    ...
    with torch.no_grad():
        for images, target in tqdm(val_loader):
            ...
            if selected_classes is not None:
                mask = np.in1d(target, selected_classes)
                target = target[mask]
                images = images[mask]
                iii += mask.sum()
            ...
            if args.use_bei_labels:
                w_mat = []
                for tar_cls in target:
                    w_mat.append(torch.Tensor(bei_dict[str(tar_cls.item())]))
                w_mat = torch.stack(w_mat).cuda()
                # print(w_mat.T.unsqueeze(2).repeat(1,1,dataset_num_classes[args.dataset]).shape)
                # print(output.softmax(dim=-1).shape)
                logit = w_mat.T.unsqueeze(2).repeat(1,1,dataset_num_classes[args.dataset]) * (output.softmax(dim=-1))
                output_mean = logit.sum(dim=0)
            elif args.uniq_bei:
                for ij, tar_cls in enumerate(target):
                    # print(ij, tar_cls)
                    tcls = tar_cls.item()
                    # print(tcls)
                    choosen_model = bei_dict[str(tcls)]
                    # print(choosen_model)
                    if choosen_model != -1:
                        # print(output.shape)
                        uniq_total[choosen_model] += 1
                        ij_pred = output[choosen_model, ij, :].argmax(-1)
                        if ij_pred == tar_cls:
                            uniq_correct[choosen_model] += 1
                        # print(uniq_correct)
                        # print(uniq_total)
                ...
                for ij, tar_cls in enumerate(target):
                    ...
                    tcls = tar_cls.item()
                    # print(tcls)
                    bei_labels = bei_dict[str(tcls)]
                    for ijj, choosen_model in enumerate(bei_labels):
                    # print(choosen_model)
                        if choosen_model != 0:
                            # print(output.shape)
                            uniq_total[ijj] += 1
                            ij_pred = output[ijj, ij, :].argmax(-1)
                            if ij_pred == tar_cls:
                                uniq_correct[ijj] += 1
                        # print(uniq_correct)
                        # print(uniq_total)
            ...
            # print(time.time() - time0)
            preds, new_target, _, acc_ub = ensem_pred(output, mode=args.ensem_pred, target=target)
    ...
    if args.uniq_bei or args.non_uniq_bei:
        print(uniq_correct)
        print(uniq_total)

        print(uniq_correct/uniq_total * 100)
