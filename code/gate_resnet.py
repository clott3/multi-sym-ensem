...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor, return_rep=False, return_both=False) -> Tensor:
        # See note [TorchScript super()]
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
    ...
    def forward(self, x: Tensor, return_rep=False, return_both=False) -> Tensor:
        return self._forward_impl(x, return_rep=return_rep, return_both=return_both)
...
def _resnet0(
    ...
    model = ResNet0(block, layers, **kwargs)
    return model
...
def resnet0(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    r"""ResNet-18 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet0("resnet0", BasicBlock, [1, 1, 1, 1], pretrained, progress, **kwargs)
