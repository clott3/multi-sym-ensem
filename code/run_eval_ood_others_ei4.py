...
perms = [[bases[i][0]+'eq', bases[i][0]+'inv', bases[i][1]+'eq', bases[i][1]+'inv'] for i in range(len(bases_perms))]
perms += [['baseline', 'baseline2', 'baseline3', 'baseline4']]
#perms =[['baseline', 'baseline2', 'baseline3', 'baseline4', 'baseline5']]
processes = []
for i in range(len(perms)):
    device = 0
    perm = perms[i]
    f = open(f'./logs_ood/simclr_{dataset}_{perm[0]}_{perm[1]}_{perm[2]}_{perm[3]}.txt', "w")
    command = f'eval_ensem.py --data /data/scratch/swhan/data/ --dataset {dataset} \
        --eval-mode freeze --combine_sep_ckpts --pretrained {models[perm[0]]} {models[perm[1]]} {models[perm[2]]} {models[perm[3]]} \
        --lr-backbone 0 --lr-classifier 0 --val-batch-size 256 \
        --exp-id simclr_{dataset}_{perm[0]}_{perm[1]}_{perm[2]}_{perm[3]} --gpu {device} \
    ...
    print(f'Queued {dataset} {perm[0]} {perm[1]} {perm[2]} {perm[3]}')
