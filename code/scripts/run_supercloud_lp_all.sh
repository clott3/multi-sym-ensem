...
for ((i=0;i<$len;i++))
do
python eval_ensem.py --submit --server=sc --dataset=$1 --arg_str="--add_prefix ${model_names[$i]} --dataset $1 --eval-mode linear_probe --batch-size=256 --lr-backbone 0.0 --lr-classifier $2 --lr-scheduler cosine --world-size 1 --rank 0 --pretrained ../pretrain_checkpoints/${model_paths[$i]}"
done