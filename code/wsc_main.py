...
# from cifar10.transforms import *
...
parser.add_argument('--dataset', type=str, default='imagenet', choices=['imagenet', 'cifar100', 'imagenet100'],
...
parser.add_argument("--server", default="wsc", type=str)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, data in enumerate(loader, start=epoch * len(loader)):
            ...
            if args.tfm == 'stylize' and args.tfm_mode == 'eq':
            ...
            if args.tfm_mode == 'inv':
                y1 = choose_transform[args.tfm](y1, args.gpu, number=1, trans_p=args.trans_p)
                y2 = choose_transform[args.tfm](y2, args.gpu, number=1, trans_p=args.trans_p)
            ...
            if args.tfm_mode == 'eq':
                rotated_images, rotated_labels = choose_transform[args.tfm](y3, args.gpu, number=args.num_eq, trans_p=args.trans_p)
            ...
            with torch.cuda.amp.autocast():
                if args.tfm_mode == 'eq':
                    loss, acc, logits = model.forward(y1, y2, labels, rotation=True, y3=rotated_images)
                else:
                    loss, acc = model.forward(y1, y2, labels)

                if args.tfm_mode == 'eq':
                    # logits = model.forward(rotated_images, rotation=True)
            ...
            if step % args.print_freq == 0:
                ...
                if args.rank == 0:
                    print(f'epoch={epoch}, step={step}, loss={loss.item()}, rotloss={float(rot_loss)}, acc={acc.item()}', flush=True)
                    stats = dict(epoch=epoch, step=step, learning_rate=lr,
                                 loss=loss.item(), rotloss=float(rot_loss), acc=acc.item(),
