...
def main():
    ...
    log_folder = os.path.join(os.getcwd(), 'checkpoint/', log_folder)
    if not os.path.exists(log_folder):
        os.makedirs(log_folder)

    job_name = os.path.basename(log_folder) if args.job_name == '' else args.job_name

   # args.cmd = args.cmd + f" --output_dir {log_folder}"
...
jsrun --bind none -E WSC=1 -n {args.nodes} -r 1 -D CUDA_VISIBLE_DEVICES -a 1 -c ALL_CPUS -g ALL_GPUS {args.cmd}
EOF
#---------------------------------------
