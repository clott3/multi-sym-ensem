...
try:
    from torchvision.datasets import Flowers102, Food101
except ImportError:
    from datasets_v08 import Flowers102, Food101
try:
...
def build_dataloaders(args):
    ...
    if args.dataset == 'imagenet':
        ...
        test_dataset = ConcatDataset(v2_datasets)

    elif args.dataset  == 'imagenet-r':
        test_dataset = ImageFolder(args.data / f'{args.dataset}', val_transforms)

    elif args.dataset == 'flowers-102':
        train_dataset = Flowers102(args.data, split='train', transform=train_transforms)
        test_dataset = Flowers102(args.data, split='test', transform=val_transforms) # may need to change to test
        ...
        test_dataset = Subset(dataset, split_idx['val'])
        test_dataset.classes = dataset.all_categories

    elif args.dataset == 'cifar10':
        train_dataset = CIFAR10(args.data, train=True, transform=train_transforms, download=True)
        test_dataset = CIFAR10(args.data, train=False, transform=val_transforms, download=True)

    elif args.dataset == 'cifar100':
        train_dataset = CIFAR100(args.data, train=True, transform=train_transforms, download=True)
        test_dataset = CIFAR100(args.data, train=False, transform=val_transforms, download=True)

    elif args.dataset == 'food':
        train_dataset = Food101(args.data, split='train', transform=train_transforms, download=True)
        test_dataset = Food101(args.data, split='test', transform=val_transforms, download=True)
    ...
    test_kwargs['batch_size'] = args.val_batch_size
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset, **test_kwargs) if test_dataset else None
    ...
    return train_loader, test_loader, indices_in_1k
